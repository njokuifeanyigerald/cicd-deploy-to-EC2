...
pipeline{
    ...
    parameters {
        ...
        string(name: 'ImageName', description: 'name of the docker build', defaultValue:"javaec2deploy")
            ...
            steps{
                ...
                script {
                    sshagent(credentials: ['ec2-cred']) {
                        ...
                        sh "ssh -o StrictHostKeyChecking=no ${REMOTE_USER}@${REMOTE_SERVER} 'docker run --name DeployedjavaApp -d -p 9099:9099 bopgeek/javaec2deploy'"
                    }
                }
            }
            post{
